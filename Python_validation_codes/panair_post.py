...
user_input = 'alpha_c/aero_current'
...
with open('./alpha_c/panair.out') as f:
    lines = f.readlines()
    ...
    for line in lines:
        ...
        if lines.index(line) == results_end:
           cl = float(lines[results_end+11][3])
           cd = float(lines[results_end+11][4])
...
with open(user_input+'_post.msh', 'a') as f:
    ...
    print('$Cl = ' + str(cl)   + 'Cdi = ' + str(cd))
    f.write('$Cl = ' + str(cl)   + 'Cdi = ' + str(cd)) 
